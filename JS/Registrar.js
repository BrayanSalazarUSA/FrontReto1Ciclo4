...
const validarPassword2 = () => {
  ...
  if (inputPassword1.value !== inputPassword2.value) {
    document.querySelector("#group4 .formulario__input-error").classList.add("formulario__input-error-activo");
    document.getElementById("passwordrepeat").classList.add("incorrecto");
  } else {
    document.querySelector("#group4 .formulario__input-error").classList.remove("formulario__input-error-activo");
...
async function emailValidation(nombre, correo, password) {
  ...
  const reponse = await fetch("http://129.151.109.250:8080/api/user/" + email);
      ...
      if (pass1 == pass2) {
        if (nombre && correo && password) {
          ...
          $.ajax({
            ...
            url: "http://129.151.109.250:8080/api/user/new",
            data: datosPeticion,
            type: "POST",
